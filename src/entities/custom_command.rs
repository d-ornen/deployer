...
impl Execute for CustomCommand {
  ...
  fn execute(&self, env: BuildEnvironment) -> anyhow::Result<(bool, Vec<String>)> {
    ...
    if self.placeholders.is_some()
      && let Some(replacements) = &self.replacements
    {
...
pub fn compose_output(
  ...
  if success && !show_success_output {
    return output;
  }
  ...
  if !stderr.trim().is_empty() {
    ...
    if total != 0 && !success {
      output.push(format!("{}", i18n::ERRORS.red().bold()));
    }
  ...
  if let Ok(num) = std::env::var("DEPLOYER_TRIM_ERR_OUT_LINES")
