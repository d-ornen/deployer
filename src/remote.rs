...
#[cfg(feature = "tui")]
pub fn edit_remote(globals: &mut DeployerGlobalConfig, args: CatRemoteArgs) -> anyhow::Result<()> {
  let remote = match globals
    .remote_hosts
    .get_mut(&ShortName::new(&args.remote_host_short_info)?)
  {
    None => exit(1),
    Some(remote) => remote,
  };
