  ...
  fn edit_from_prompt(&mut self, opts: &mut DeployerGlobalConfig) -> anyhow::Result<()> {
    ...
    .prompt_skippable()?
    {
      match action {
        ...
        i18n::EDIT_DEPL_TOOLKIT => {
          self.deploy_toolkit =
            inquire::Text::new(&format!("{} {}:", i18n::DEPL_TOOLKIT, i18n::OR_HIT_ESC)).prompt_skippable()?
        }
        ...
        i18n::EDIT_AF_INPLACE => self
          .place_artifacts_into_project_root
          .edit_from_prompt(&mut self.artifacts)?,
  ...
  pub fn select_default_pipeline(&mut self) -> anyhow::Result<()> {
    match self.pipelines.len() {
      ...
      _ => {
        ...
        if let Some(pipe) =
          inquire::Select::new(&format!("{} {}:", i18n::EDIT_DEFAULT_PROMPT, i18n::HIT_ESC), cs).prompt_skippable()?
        {
...
impl EditExtended<DeployerGlobalConfig> for Vec<DescribedAction> {
  fn edit_from_prompt(&mut self, opts: &mut DeployerGlobalConfig) -> anyhow::Result<()> {
    loop {
      ...
      self.iter_mut().for_each(|c| {
        let s = i18n::ACTION_EDIT
          .replace("{1}", &c.title)
          .replace("{2}", &c.info.to_str());
      ...
      cs.extend_from_slice(&[
        i18n::REORDER.to_string(),
        i18n::ADD.to_string(),
        i18n::REMOVE.to_string(),
      ]);

      if let Some(action) =
        inquire::Select::new(&format!("{} {}:", i18n::ACTION_SELECT_TO_CHANGE, i18n::HIT_ESC), cs).prompt_skippable()?
      {
        match action.as_str() {
  ...
  fn reorder(&mut self, _opts: &mut DeployerGlobalConfig) -> anyhow::Result<()> {
    ...
    for selected in self.iter() {
      let key = i18n::ACTION
        .replace("{1}", &selected.title)
        .replace("{2}", &selected.info.to_str());
      k.push(key.clone());
  ...
  fn add_item(&mut self, opts: &mut DeployerGlobalConfig) -> anyhow::Result<()> {
    ...
    for action in opts.actions_registry.values() {
      let key = i18n::ACTION
        .replace("{1}", &action.title)
        .replace("{2}", &action.info.to_str());
      k.push(key.clone());
      h.insert(key, action);
    }
...
impl DescribedAction {
  pub fn edit_action_from_prompt(&mut self) -> anyhow::Result<()> {
    ...
    .prompt_skippable()?
    {
      match action {
        i18n::EDIT_TITLE => {
          self.title = inquire::Text::new(i18n::ACTION_FULL_NAME)
            .with_initial_value(self.title.as_str())
            .prompt()?
        }
        i18n::EDIT_DESC => {
          self.desc = inquire::Text::new(i18n::ACTION_DESC)
            .with_initial_value(self.desc.as_str())
            .prompt()?
        }
        i18n::EDIT_TAGS => {
        ...
        i18n::EDIT_COMMANDS => match &mut self.action {
        ...
        i18n::EDIT_TARGETS => match &mut self.action {
          Action::Pack(a) | Action::Deliver(a) | Action::Install(a) => {
            a.target = Some(TargetDescription::new_from_prompt()?);
          }
          _ => {}
        },
        i18n::EDIT_DEPL_TOOLKIT => match &mut self.action {
          Action::ConfigureDeploy(a) | Action::Deploy(a) | Action::PostDeploy(a) => {
            a.deploy_toolkit =
              inquire::Text::new(&format!("{} {}:", i18n::DEPL_TOOLKIT, i18n::HIT_ESC)).prompt_skippable()?;
          }
          _ => {}
        },
        i18n::EDIT_PATCH if let Action::Patch(patch) = &mut self.action => {
        ...
        i18n::EDIT_EXEC_IN_PROJECT_DIR => {
          self.exec_in_project_dir = if let Ok(Some(exec_in_project_dir)) =
            inquire::Confirm::new(i18n::EXEC_IN_PROJECT_DIR)
...
impl DescribedPipeline {
  pub fn edit_pipeline_from_prompt(&mut self, globals: &mut DeployerGlobalConfig) -> anyhow::Result<()> {
    ...
    .prompt_skippable()?
    {
      match action {
        i18n::EDIT_TITLE => {
          self.title = inquire::Text::new(i18n::PIPELINE_FULL_NAME)
            .with_initial_value(self.title.as_str())
            .prompt()?
        }
        i18n::EDIT_DESC => {
          self.desc = inquire::Text::new(i18n::PIPELINE_DESC)
            .with_initial_value(self.desc.as_str())
            .prompt()?
        }
        i18n::EDIT_TAGS => {
        ...
        i18n::EDIT_EXCL_TAG => {
          self.exclusive_exec_tag = if self.exclusive_exec_tag.is_none() {
            inquire::Text::new(&format!("{} {}:", i18n::PIPELINE_SPECIFY_EXCL_TAG, i18n::OR_HIT_ESC))
              .prompt_skippable()?
          } else {
            inquire::Text::new(&format!("{} {}:", i18n::PIPELINE_SPECIFY_EXCL_TAG, i18n::OR_HIT_ESC))
              .with_initial_value(self.exclusive_exec_tag.as_ref().unwrap())
              .prompt_skippable()?
          }
        }
        _ => {}
...
impl EditExtended<DeployerGlobalConfig> for Vec<DescribedPipeline> {
  fn edit_from_prompt(&mut self, opts: &mut DeployerGlobalConfig) -> anyhow::Result<()> {
    loop {
      ...
      if let Some(action) = inquire::Select::new(&format!("{} {}:", i18n::SELECT_PIPELINE_TO_CHANGE, i18n::HIT_ESC), cs)
        .prompt_skippable()?
      {
        match action.as_str() {
          i18n::REORDER => self.reorder(opts)?,
  ...
  fn add_item(&mut self, opts: &mut DeployerGlobalConfig) -> anyhow::Result<()> {
    ...
    for pipeline in opts.pipelines_registry.values() {
      let key = i18n::PIPELINE
        .replace("{1}", &pipeline.title)
        .replace("{2}", &pipeline.info.to_str());
      k.push(key.clone());
    ...
    if selected.as_str() == USE_ANOTHER {
      ...
      self.push(
        (**h
          .get(&selected)
          .ok_or(anyhow::anyhow!("Can't get specified Pipeline!"))?)
        .clone(),
      );
    }
    Ok(())
...
impl Edit for Vec<Requirement> {
  fn edit_from_prompt(&mut self) -> anyhow::Result<()> {
    loop {
      ...
      if let Some(action) =
        inquire::Select::new(&format!("{} {}:", i18n::EDIT_ACTION_PROMPT, i18n::HIT_ESC), cs).prompt_skippable()?
      {
        match action.as_str() {
...
impl EditExtended<PathType> for Vec<PathBuf> {
  ...
  fn reorder(&mut self, _: &mut PathType) -> anyhow::Result<()> {
    Ok(())
  }
...
impl EditExtended<Vec<PathBuf>> for Vec<(PathBuf, PathBuf)> {
  ...
  fn reorder(&mut self, _opts: &mut Vec<PathBuf>) -> anyhow::Result<()> {
    Ok(())
  }
...
impl Edit for Vec<TargetDescription> {
  fn edit_from_prompt(&mut self) -> anyhow::Result<()> {
    loop {
      ...
      if let Some(action) =
        inquire::Select::new(&format!("{} {}:", i18n::SELECT_TARGET_TO_CHANGE, i18n::HIT_ESC), cs).prompt_skippable()?
      {
        match action.as_str() {
          i18n::ADD => self.add_item()?,
          i18n::REMOVE => self.remove_item()?,
...
impl Variable {
  pub fn edit_variable_from_prompt(&mut self) -> anyhow::Result<()> {
    let actions = vec![i18n::EDIT_TITLE, i18n::EDIT_VAR_SECRET, i18n::EDIT_VALUE];
    ...
    .prompt_skippable()?
    {
      match action {
        ...
        i18n::EDIT_VAR_SECRET => {
          self.is_secret = inquire::Confirm::new(i18n::VAR_IS_SECRET)
            .with_default(false)
            .prompt()?
        }
...
impl Edit for Vec<Variable> {
  fn edit_from_prompt(&mut self) -> anyhow::Result<()> {
    loop {
      ...
      if let Some(action) =
        inquire::Select::new(&format!("{} {}:", i18n::VAR_SELECT_FC, i18n::HIT_ESC), cs).prompt_skippable()?
      {
        match action.as_str() {
          i18n::ADD => self.add_item()?,
          i18n::REMOVE => self.remove_item()?,
...
impl TargetDescription {
  pub fn edit_target_from_prompt(&mut self) -> anyhow::Result<()> {
    let actions = vec![i18n::EDIT_ARCH, i18n::EDIT_OS];
...
impl Edit for CustomCommand {
  fn edit_from_prompt(&mut self) -> anyhow::Result<()> {
    while let Some(action) = inquire::Select::new(
      &format!(
        "{} {}:",
        i18n::CMD_SELECT_TO_CHANGE.replace("{}", &self.bash_c.green()),
        i18n::HIT_ESC
      ),
      vec![
        i18n::CMD_EDIT_SHELL,
    ...
    .prompt_skippable()?
    {
      match action {
        ...
        i18n::CMD_CHANGE_PLACEHOLDERS => {
          ...
          self.placeholders = if placeholders.is_empty() {
            None
          } else {
            Some(placeholders)
          };
...
impl Edit for Vec<CustomCommand> {
  fn edit_from_prompt(&mut self) -> anyhow::Result<()> {
    loop {
      ...
      cs.extend_from_slice(&[
        i18n::CUSTOM_CMD_REORDER.to_string(),
        i18n::CUSTOM_CMD_ADD.to_string(),
        i18n::CUSTOM_CMD_RM.to_string(),
      ]);

      if let Some(action) =
        inquire::Select::new(&format!("{} {}:", i18n::CUSTOM_CMD_EDIT_PROMPT, i18n::HIT_ESC), cs).prompt_skippable()?
      {
        match action.as_str() {
...
impl Edit for Vec<ProgrammingLanguage> {
  fn edit_from_prompt(&mut self) -> anyhow::Result<()> {
    loop {
      ...
      if let Some(action) =
        inquire::Select::new(&format!("{} {}:", i18n::PL_ACTION_PROMPT, i18n::HIT_ESC), cs).prompt_skippable()?
      {
        match action.as_str() {
          i18n::ADD => self.add_item()?,
          i18n::REMOVE => self.remove_item()?,
...
impl CheckAction {
  pub fn change_regexes_from_prompt(&mut self) -> anyhow::Result<()> {
    ...
    loop {
      ...
      if self.success_when_found.is_some() || self.success_when_not_found.is_some() {
        break;
      } else {
...
impl Edit for PatchAction {
  fn edit_from_prompt(&mut self) -> anyhow::Result<()> {
    self.patch = PathBuf::from(
      inquire::Text::new(i18n::PATCH_SPECIFY_PATH)
...
impl Edit for AddToStorageAction {
  fn edit_from_prompt(&mut self) -> anyhow::Result<()> {
    loop {
      let short_name = inquire::Text::new(i18n::SPECIFY_SHORT_NAME_FOR_ADD_TO_STORAGE)
...
impl Edit for RemoteHost {
  fn edit_from_prompt(&mut self) -> anyhow::Result<()> {
    ...
    self.username = inquire::Text::new(i18n::SPECIFY_HOST_USERNAME)
      ...
      .prompt()?;
    self.ssh_private_key_file = PathBuf::from(
      inquire::Text::new(i18n::SPECIFY_SSH_KEY_PATH)
